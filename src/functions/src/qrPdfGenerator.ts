...
export const generateQrPdfBuffer = async (qrCodes: any[]): Promise<Buffer> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < qrCodes.length; i++) {
        ...
        const qrDataUrl = await QRCode.toDataURL(code.label, {
        ...
        doc.fontSize(labelFontSize).text(
          code.label,
          x,
          qrY + qrCodeSize + 5, // Position text below the QR code
          {
            align: 'center',
            width: cellWidth,
          }
