...
function RefundsStep1Page(): React.JSX.Element {
  ...
  const handleCodeScanned = async (data: string) => {
    try {
      ...
      if (result.data) {
        ...
        navigate(`/home/sales/refunds/step2?code=${encodeURIComponent(data)}`);
      } else {
        // Invalid QR code
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data) {
        // Valid QR code, navigate to step 2
        navigate(`/home/sales/refunds/step2?code=${encodeURIComponent(manualCode)}`);
      } else {
        // Invalid QR code
        setError('Invalid QR code. Please try again.');
      }
  ...
  return (
    <>
      <div className="p-4">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Refund - Step 1</h1>
        <p className="text-gray-600 mb-6">Scan or enter a QR code to begin a refund transaction.</p>
