...
const TotalSalesCard: React.FC<{ totalCents: number; onRefresh: () => void }> = ({ totalCents, onRefresh }) => {
    ...
    <div className="bg-white rounded-lg shadow-md p-4 mb-4">
      <div className="flex justify-between items-start">
        ...
        <button
          ...
          aria-label={t('salesPage.refresh')}
        >
...
function SalesPage(): React.JSX.Element {
  ...
  // Function to refresh stall data
  const refreshStallData = () => {
    if (currentStallId) {
  ...
  // Invalidate queries when component mounts
  useEffect(() => {
    refreshStallData();
  }, [queryClient, currentStallId]);
  
  // Additional effect for when page becomes visible again
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 flex-shrink-0">
        <TotalSalesCard totalCents={totalSalesCents} onRefresh={refreshStallData} />
      </div>
